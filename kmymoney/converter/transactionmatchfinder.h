...
protected:
  ...
  /** Checks whether payees of both splits match each other or at least one of them is empty, 
   * OR that their notes match (special case where notes contain the original payee)
   * @param split1 the first split
   * @param split2 the second split
   * @return true, if splits reference the same payee or at least one payee is empty, or notes match; false otherwise
   */
  bool splitsPayeesOrNotesMatchOrEmpty(const MyMoneySplit & split1, const MyMoneySplit & split2) const;
